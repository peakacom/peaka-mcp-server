...
const enabledTools: Record<string, boolean> = {
  peaka_schema_retriever: false,
  peaka_query_golden_sqls: true,
  peaka_execute_sql_query: true,
  peaka_get_project_metadata: true,
  peaka_list_catalogs: false,
  peaka_list_schemas: false,
  peaka_list_tables: true,
  peaka_list_columns: false,
  peaka_create_cache: true,
  peaka_get_cache_statuses: true,
  peaka_list_queries: true,
  peaka_execute_query: true,
  peaka_list_projects: true,
  peaka_select_project: true,
  peaka_refresh_project_metadata: true,
  peaka_get_metadata_refresh_status: true,
};

if (enabledTools.peaka_schema_retriever) {
  server.addTool({
    name: "peaka_schema_retriever",
    description:
      `Retrieve table metadata and schema.
    Metadata has column types and relationships of the table with other tables.
    Use this tool if you can't find a golden query and you have to use the query yourself.`,
    parameters: z.object({
...
if (enabledTools.peaka_query_golden_sqls) {
  server.addTool({
...
if (enabledTools.peaka_execute_sql_query) {
  server.addTool({
...
if (enabledTools.peaka_get_project_metadata) {
  server.addTool({
...
if (enabledTools.peaka_list_catalogs) {
  server.addTool({
...
if (enabledTools.peaka_list_schemas) {
  server.addTool({
...
if (enabledTools.peaka_list_tables) {
  server.addTool({
...
if (enabledTools.peaka_list_columns) {
  server.addTool({
...
if (enabledTools.peaka_create_cache) {
  server.addTool({
...
if (enabledTools.peaka_get_cache_statuses) {
  server.addTool({
...
if (enabledTools.peaka_list_queries) {
  server.addTool({
...
if (enabledTools.peaka_execute_query) {
  server.addTool({
...
if (enabledTools.peaka_list_projects) {
  server.addTool({
...
if (enabledTools.peaka_select_project) {
  server.addTool({
...
if (enabledTools.peaka_refresh_project_metadata) {
  server.addTool({
...
if (enabledTools.peaka_get_metadata_refresh_status) {
  server.addTool({
